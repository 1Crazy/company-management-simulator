...
<template>
  <section class="grid gap-3 md:grid-cols-4">
    <article class="panel-surface panel-glow px-4 py-4">
      ...
      <div class="mono-title mt-2 text-2xl text-[var(--control-navy)]">{{ formatCurrency(session.state.cash) }}</div>
      <div
        class="mt-2 text-sm"
    ...
    <article class="panel-surface panel-glow px-4 py-4">
      ...
      <div class="mono-title mt-2 text-2xl text-[var(--control-navy)]">{{ formatCurrency(session.state.accumulatedProfit) }}</div>
      <div
        class="mt-2 text-sm"
        :class="
    ...
    <article class="panel-surface panel-glow px-4 py-4">
      ...
      <div class="mono-title mt-2 text-2xl text-[var(--control-navy)]">{{ formatPercent(session.state.marketShare) }}</div>
    ...
    <article class="panel-surface panel-glow px-4 py-4">
      ...
      <div class="mono-title mt-2 text-2xl text-[var(--control-navy)]">{{ Math.round(session.state.quality) }} / {{ Math.round(session.state.morale) }}</div>
